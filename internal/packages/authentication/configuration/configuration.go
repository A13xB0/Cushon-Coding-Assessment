// This is the configuration package for the authentication service. This is managed by flags but if I had more time and this was a production project I would use VIPER so we can take in
// env variables instead of having to convert from env variable to flags via a start.sh in the container.
package AuthConfiguration
...
func GetConfig() AuthConfig {
	...
	webServicePort := flag.Int("webserverhost", 8080, "Web Hostname")
	mac := flag.String("mac", "K1ng5l3y", "Message Authentication Code, used for authentication")
	loggerServiceName := flag.String("servicename", "authenticator-1", "The service name used for the logger")
